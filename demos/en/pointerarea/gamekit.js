...
gamekit.Emitter = Emitter = function (conf){
	...
	this.particlePreset = {
		minDirection:    conf.minDirection || 0,
		maxDirection:    conf.maxDirection || 0,
		minSpeed:        conf.minSpeed || 1,
		maxSpeed:        conf.maxSpeed || 1,
		speedRandomness: conf.speedRandomness || 0,
		minLife:         conf.minLife || 100,
		maxLife:         conf.maxLife || 100,
		minWidth:        conf.minWidth || 10,
		maxWidth:        conf.maxWidth || 10,
		minAlpha:        conf.minAlpha || 1,
		maxAlpha:        conf.maxAlpha || 1
	};
...
Emitter.prototype = {
	...
	recycle: function (particle){
		...
		particle.w = particle.h = gamekit.randomInRange(pre.minWidth, pre.maxWidth);
		particle.alpha = 0;
		particle.targetAlpha = gamekit.randomInRange(pre.minAlpha, pre.maxAlpha);
		...
		particle.direction = gamekit.randomInRange(pre.minDirection, pre.maxDirection);
		particle.speed = gamekit.randomInRange(pre.minSpeed, pre.maxSpeed);
	}
};
...
gamekit.Particle = Particle = function (emitter){
	...
	var sprt = emitter.assets[Math.round(gamekit.randomInRange(0, emitter.assets.length - 1))];

	gamekit.Sprite.call(this, gamekit.a[sprt]);
...
Particle.prototype = gamekit.extend(gamekit.Sprite.prototype, {
	update: function (){
		...
		if(this.life > 0 && this.alpha < this.targetAlpha){
			this.alpha += 0.005;
		}
